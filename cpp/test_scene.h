...
vec3 shade_nolight(const Ray& r, Hitable *world, int depth) {
    HitRecord rec;
    if(world->hit(r, 0.001, MAXFLOAT, rec)) {
        Ray scattered;
        vec3 attenuation;
        if(depth < 15 && rec.mat_ptr->scatter(r, rec, attenuation, scattered)) {
            return attenuation * shade_nolight(scattered, world, depth + 1);
        }
        else {
            return vec3(0, 0, 0);
        }
...
Hitable* draw_one_triangle() {

    int l = 0;
    Hitable** list = new Hitable*[8];

    vec3 vertices[3] = {vec3(0, 10, -10), vec3(10, -6, -10), vec3(-10, -6, -10)};
    list[l++] = new Triangle(vertices, new DiffuseLight(new ConstantTexture(vec3(0.4, 0.7, 0.5))), false);

    return new HitableList(list, l);
}


// Hitable* draw_one_mesh() {

//     int l = 0;
//     Hitable** list = new Hitable*[1];

//     Material* light = new DiffuseLight(new ConstantTexture(vec3(7, 7, 7)));
//     list[l++] = new RectangleXZ(-10, 10, -10, 10, 15, light);

//     std::string fn = "../shapes/bunny.obj";
//     list[l++] = new Mesh(fn, new Metal(vec3(0.5*(1+drand48()), 0.5*(1+drand48()), 0.5*(1+drand48())), 0.5*drand48()));
